...
把限定原子层词汇内的可编辑多层材质栈，编译成固定成本、可直接被实时光照积分的 closure packet，并验证它在未见层组合上接近随机游走参考。

近期工作始终围绕最短研究闭环展开：

1. 可信的多层随机游走 teacher；
2. teacher 生成的方向响应数据；
3. 用逐 tile 直接优化的 oracle 实验确定固定成本的 closure 表示；
4. 训练通用编译器；
5. 把最终参数包接入 Falcor 延迟着色。

当前已实现的基线是“精确计算顶层界面直接反射，再用两个 LTC 瓣拟合剩余响应”。它的方向域相对 L1 误差中位数为 6.73%，第 90 百分位为 31.20%，长尾仍然偏高。任何文档不得再把“闭包表示问题已经解决”当作既定结论。

当前任务顺序是：先降低粗糙导体基底和深层栈的表示长尾，确认网络最终需要输出的参数；再实现结构化层组合网络。不要在输出表示仍可能变化时提前固化最终网络或渲染接口。
...
- `teacher`、`oracle`、`closure packet` 等项目内常用名称可以保留，但正文要分别说明为“随机游走参考解”“逐样本直接优化得到的表示上界”“固定大小的解析着色参数包”。

## 根 Git 仓库边界

- 根仓库包含项目自有源码、测试、环境声明、中文文档、轻量 Markdown/JSON 实验结果和资产清单。
- 根仓库不包含 `external/`、`data/`、`build/`、缓存，以及报告中的 `.npy`、`.npz`、`.pt` 等可重新生成的二进制。
