...
from .packet import ClosurePacket
...
class ClosurePacketTensors:
    layer_type: torch.Tensor
...
def packets_to_tensors(
    packets: Sequence[ClosurePacket],
    *,
    device: str | torch.device | None = None,
) -> ClosurePacketTensors:
    torch_device = torch.device(device or "cpu")
    layers = [packet.direct_top for packet in packets]
    return ClosurePacketTensors(
        layer_type=torch.tensor([int(layer.layer_type) for layer in layers], device=torch_device),
        roughness=torch.tensor(
            [[layer.roughness_x, layer.roughness_y] for layer in layers],
            dtype=torch.float32,
            device=torch_device,
        ),
        eta=torch.tensor([layer.eta for layer in layers], dtype=torch.float32, device=torch_device),
        k=torch.tensor([layer.k for layer in layers], dtype=torch.float32, device=torch_device),
        albedo=torch.tensor(
            [layer.albedo for layer in layers], dtype=torch.float32, device=torch_device
        ),
        tangent_rotation=torch.tensor(
            [layer.tangent_rotation for layer in layers], dtype=torch.float32, device=torch_device
        ),
        amplitude=torch.tensor(
            [[lobe.amplitude for lobe in packet.residual_lobes] for packet in packets],
            dtype=torch.float32,
            device=torch_device,
        ),
        inverse_scale=torch.tensor(
            [[lobe.inverse_scale for lobe in packet.residual_lobes] for packet in packets],
            dtype=torch.float32,
            device=torch_device,
        ),
        shear=torch.tensor(
            [[lobe.shear for lobe in packet.residual_lobes] for packet in packets],
            dtype=torch.float32,
            device=torch_device,
        ),
        angle=torch.tensor(
            [[lobe.angle for lobe in packet.residual_lobes] for packet in packets],
            dtype=torch.float32,
            device=torch_device,
        ),
    )
...
def eval_direct_top(
    ...
    view = _to_layer_frame(view_directions, packet.tangent_rotation)
    lights = light_directions[None, :, :].expand(tile_count, -1, -1)
    light = _to_layer_frame(lights, packet.tangent_rotation[:, None])
    wi = view[:, None, :].expand_as(light)
    positive = (wi[..., 2] > 1e-6) & (light[..., 2] > 1e-6)
    alpha = torch.clamp(packet.roughness, min=1e-3)[:, None, :]
    ...
    diffuse = packet.albedo[:, None, :] / math.pi

    roughness = torch.clamp(packet.roughness[:, 0:1], min=1e-3)
    ...
    sheen = packet.albedo[:, None, :] * (
        charlie_d * sheen_g / denominator
    )[..., None]

    layer_type = packet.layer_type[:, None, None]
    result = torch.where(
        layer_type == 0,
        dielectric,
...
def evaluate_closure_packet(
    ...
    return eval_direct_top(packet, view_directions, light_directions) + eval_ltc_residual(
        light_directions,
        packet.amplitude,
        packet.inverse_scale,
        packet.shear,
        packet.angle,
    )
