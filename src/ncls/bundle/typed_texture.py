...
def encode_rgba16f_dds(levels: Sequence[np.ndarray]) -> bytes:
    if not levels:
        raise ValueError("RGBA16F DDS requires at least one mip level")
    normalized = tuple(np.asarray(level, dtype="<f2") for level in levels)
    height, width = normalized[0].shape[:2]
    if height < 1 or width < 1:
        raise ValueError("RGBA16F DDS base extent must be positive")
    expected_height, expected_width = height, width
    for level in normalized:
        if level.shape != (expected_height, expected_width, 4):
            raise ValueError("RGBA16F DDS mip extents/channels are inconsistent")
        if not np.isfinite(level).all():
            raise ValueError("RGBA16F DDS texels must be finite")
    ...
    struct.pack_into(
        "<7I", header, 4,
        124, flags, height, width, width * 8, 0, len(normalized),
    )
...
def inspect_rgba16f_dds(payload: bytes) -> tuple[int, int, int]:
    ...
    dxgi_format, dimension, misc, array_size, misc2 = struct.unpack_from("<5I", payload, 128)
    if (
        size != 124 or pixel_size != 32 or pixel_flags != 0x4 or fourcc != _DX10_FOURCC
        or dxgi_format != _DXGI_FORMAT_R16G16B16A16_FLOAT
        or dimension != _D3D10_RESOURCE_DIMENSION_TEXTURE2D
        or misc != 0 or array_size != 1 or misc2 != 0 or depth != 0
        or width < 1 or height < 1 or mip_count < 1 or pitch != width * 8
    ):
        raise ValueError("RGBA16F DDS header disagrees with the typed texture contract")
    expected_bytes = 148
    mip_width, mip_height = width, height
    for _ in range(mip_count):
        expected_bytes += mip_width * mip_height * 8
        mip_width, mip_height = max(1, mip_width // 2), max(1, mip_height // 2)
    if len(payload) != expected_bytes:
        raise ValueError("RGBA16F DDS payload length disagrees with its mip chain")
...
def validate_typed_resource(payload: bytes, descriptor: Mapping[str, Any]) -> None:
    ...
    if int(descriptor.get("stride", 0)) != 8 or int(descriptor.get("alignment", 0)) < 16:
        raise ValueError("RGBA16F DDS descriptor stride/alignment is invalid")
    usage = descriptor.get("usage")
    if not isinstance(usage, str) or not usage:
        raise ValueError("RGBA16F DDS descriptor requires a shader usage")
