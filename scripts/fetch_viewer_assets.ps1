...
$presetPath = Join-Path $projectRoot "configs\viewer-studio-v2.json"
...
$sceneSource = Join-Path $projectRoot "external\MaterialX\resources\Geometry\shaderball.glb"
$sceneTargetDirectory = Join-Path $projectRoot "assets\viewer\scenes\studio-v1"
$sceneTarget = Join-Path $projectRoot ($preset.scene.geometry -replace '/', '\')
Assert-Sha256 $sceneSource $preset.scene.geometry_sha256 "Pinned MaterialX shaderball"
...
Assert-Sha256 $sceneTarget $preset.scene.geometry_sha256 "Provisioned viewer shaderball"

$environmentTarget = Join-Path $projectRoot ($preset.scene.environment -replace '/', '\')
...
if ($environmentValid) {
    $environmentValid = (Get-FileHash -Algorithm SHA256 -LiteralPath $environmentTarget).Hash.ToLowerInvariant() `
        -eq $preset.scene.environment_sha256.ToLowerInvariant()
}
...
Assert-Sha256 $environmentTarget $preset.scene.environment_sha256 "Provisioned viewer HDRI"
Write-Output "Viewer studio assets verified."
