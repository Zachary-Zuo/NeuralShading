...
- `scope`：新增独立`metal_budgeted_hybrid_dual_local_v6` pilot profile。两张RGBA plane都使用请求texel、signed x/y中心差分和role embedding组成的20维per-slot输入，保留v3 shared-slot softmax；第二张plane由1/4线性分辨率提升到全分辨率。asset encoder参数shape保持与公共method descriptor一致；semantic/evaluator、参数化PreparedState和两次读取均不变。
...
- `implementation correction`：旧schedule `group-block-balanced@1`在一个validation window内只选每rank一个group，且group随checkpoint step变化，因而`mixed-cohort-v2`的step轨迹不是同cohort比较。该证据使原“直接比较v2四个milestone”的协议假设失效。新`group-block-balanced@2`只改变多group validation cohort：engine为窗口内batch写`validation_group_index=0..255`，producer每64 batch切group；训练schedule、source、loss、optimizer、batch和hard budget不变。旧run不覆盖，`mixed-cohort-v3`在新config identity下fresh重跑。
- `outcome`：v3在DDP5上完成512 step，checkpoint为`61a2b4aae12fa3a1bdd222f51139b6f5092ec66568ef496fbde01b672692b656`。20-group同分布validation的appearance/log/linear/chroma改善，但peak退化、spatial仅改善约`0.00217`；四个exact-query group block中spatial反而退化约`0.000104`，预测gradient保持在target约2%。结果支持“平均response可学而局部空间映射仍失败”，但不同group的linear/peak方向不一致，因此不宣称692-source普适质量。
