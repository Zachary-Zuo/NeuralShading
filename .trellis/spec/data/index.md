...
项目没有离线训练数据层或 corpus sink。新增 source 必须提供 locator loader、canonical snapshot、typed editor、完整 `prepare/evaluate/sample/pdf` reference 与 parity；新增 method 只通过 data facet 声明数据需求和 source adapter。不得增加 collector、磁盘 batch、reader、family-specific producer 或 method-specific data loop。
