...
新增或修改`Method`、method descriptor、训练objective、artifact compiler、`ScatteringPackage`、评测或viewer部署时适用。公共runner、bundle writer和viewer不得通过concrete method名称补齐缺失语义。

## 2. Signatures

```text
Method.descriptor -> MethodDescriptor@2
...
method.compile_asset(source_snapshot, checkpoint) -> AssetPayload
...
- 当前 Metal budgeted 方法按实际 shader 能力导出 prepare/evaluate/sample/pdf；普通导出不要求 formal、complete 或梯度覆盖，当前训练 step 和诊断写入 metadata。
...
| 初始化或短训 checkpoint | 正常导出当前模型，记录实际诊断，不增加 readiness 门禁 |
...
- Good：初始化模型与短训模型使用同一个 compiler，能生成与当前结构对应的 package。
...
- integration：当前 Metal checkpoint 依次检查量化 Python、Slang package 与 viewer，能力与研究标签分离。
...
artifacts = method.compile_asset(source_snapshot, checkpoint)
