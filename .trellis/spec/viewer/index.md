...
质量门：slot/capture schema unit tests、package hash/ABI 失败矩阵、五种 source reference 与 NVIDIA 的 PT/deferred 组合、Release build、Falcor worktree clean。MDL 的动态 artifact 路径还必须验证 compiler/file identity、V1 capability、formal boundary、matched sample/PDF 一致性，以及 car paint/ceramic 的真实 headless firefly 尾部。
