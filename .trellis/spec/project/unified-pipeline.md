...
`SourceSnapshot` 是 source 唯一真相；batch 全 tensor 同 device；live target 禁止 host readback；checkpoint tensor schema 来自 method descriptor；package 三个 identity 独立；viewer 恰有两个对称 slot，panel 宽度为 `floor(W/2)`。
...
unit 覆盖 batch、method fixture、checkpoint、package tamper、source editor、slot extent/studio；Falcor 覆盖 CUDA live batch 与 package absolute-path compile；Release viewer build 后确认 Falcor clean。
