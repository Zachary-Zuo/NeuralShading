...
MDL V1的runtime reference与其他source一样完整实现canonical `prepare/evaluate/sample/pdf`，四个入口都落到同一compiled target code；缺任一入口时descriptor fail closed。online训练通过统一dispatcher调用其中的`prepare/evaluate`，source `sample/pdf`用于transport与数值验证。不存在MDL专用provider/query/batch入口，也不通过viewer adapter或fallback拼接能力。
