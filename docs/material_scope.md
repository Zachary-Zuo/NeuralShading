...
MDL source family 保存 module、exact export signature、class-compiled typed arguments、传递依赖与原生纹理资源。MDL SDK 是语言、标准 closure 与 target code 的上游语义核心；项目 bridge 和当前 Falcor 8 是唯一正式执行路径。锁定 falcor2 使用同版 MDL SDK，只用于 renderer integration parity，不是第二条 GT 或运行时 fallback。MDL V1 对外只声明 surface-BSDF `evaluate`，纹理查询固定 `ExplicitLod(0)`；viewer/image parity 与 derivative filtering 在各自实现和验收完成前保持未声明。
