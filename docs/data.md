...
`ncls.e2-layer-stack-shared-decoder@1` 是 E2 的版本化共享表示 profile：固定 12 个材质族、每族 2 个保持相同图结构的局部参数状态。前 8 个族分别固定 1–8 个界面，另外 4 个族继续从研究 prior 采样；family 粒度划分产生 20/2/2 个 train/validation/test 状态，禁止同族局部状态跨 split。它只定义 LayerStack source distribution，不向公共 response reader 暴露 `LayerStackIR`。当前 E2 使用 `ncls.e2-layer-stack-independent-peak-grazing-mixture@3`：每个 query role 明确锚定一个掠射 `wo`；单界面 sheen 由 LayerStack adapter 按 shader 的 `(viewDirection, lightDirection)` 约定和权威 response measure `f(wo,wi) * |cos θ_i|` 求 Charlie 峰中心；其余状态围绕几何镜面方向布置径向 ±12.6 度、方位 ±3.6 度的 75 点窄 vMF patch，并另外保留 34 个 uniform 与 19 个 grazing query。这个 patch 是由多界面 response 峰相对镜面方向移动 4–11 度的高分辨率 probe 得到的最小覆盖修正，不假设能从 LayerStack source state 解析出全局峰，也不向公共 query 接口暴露层参数。每个落盘方向仍保存整个 mixture 的连续归一化 PDF 和 `1/(N p)` 权重。若 audit 证明某个 split group 已打满公共 adaptive 上限，CLI 可用 `--adaptive-max-samples-by-split-group GROUP=SAMPLES` 只提高该组上限；映射会写入 provider metadata，未知 group、重复 group、非 batch 整数倍或低于公共上限的值都拒绝。`configs/research/e2-supervision-gates-v1.json` 复现原镜面中心失败，v2 复现错误 response measure/参数约定，v3 则保留了多界面 moving-peak 漏洞；正式 v5 dataset `bbcd51e7451e7e2b8df705abc4eeb1382684c37bce1152284d9c4900dc5fd515` 使用仅限 `family-0002/0003` 的 262,144 samples/replica 上限，通过 `configs/research/e2-supervision-gates-v4.json` 全部检查，当前允许用于 optimized latent、target encoder 与 shared decoder 的比较。
