...
所有source reference与neural runtime共享canonical `prepare/evaluate/sample/pdf`合同，但各自保留backend-specific state和交付身份。source family先从原生locator构造`SourceSnapshot`，reference program再产出typed runtime/material payload；公共backend/session只调用合同，不按LayerStack、MaterialX、MDL等family分支。
