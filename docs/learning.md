...
每次训练同时生成`.metrics.jsonl`、`.summary.json`与`.review.json`；review的初尾window/bootstrap、耗时和内存只作观察，不自动触发后续实验。smoke只证明真实full method shape下的数值、梯度、phase transition和短程收敛；不作为formal质量结论。部署由同一checkpoint编译`program/asset/instance`三段`ScatteringPackage@2`，并通过generic artifact conformance和packed-FP16 D3D12 parity。Metal的Windows验证、692-export Linux smoke/long配置与恢复命令见[Metal Linux训练交接](metal_linux_training.md)。
