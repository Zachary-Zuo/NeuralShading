...
> **历史快照说明（2026-08-23）：** 本文原文用于追溯早期“把复杂材质编译成固定解析 closure”路线，不再承担当前问题定义、研究顺序或验收条件。当前目标是以小型 MLP 直接实现 `evaluate(wo, wi)` 的 neural material program；`prepare` 负责共享编码，matched `sample/pdf` 和环境积分在 evaluator 成形后研究。以 [`docs/realtime_material_compilation.md`](../realtime_material_compilation.md) 为准，正文保留当时判断，不逐段改写。
