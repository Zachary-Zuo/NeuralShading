...
> **历史快照说明（2026-08-23）：** 本文记录最早三分调研中的完整推导，正文中的固定 closure、阶段性 claim、旧任务顺序和 kill test 不再作为当前决定。最新目标是把原生材质编译为随机访问、有界成本的 neural material program，以小型 MLP 直接执行 `evaluate(wo, wi)`；`prepare` 只形成共享编码，matched sampler 与环境积分在 evaluator 定型后展开。当前稳定表述见 [`docs/realtime_material_compilation.md`](../realtime_material_compilation.md)。
