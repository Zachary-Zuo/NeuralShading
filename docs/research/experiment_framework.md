...
本框架取代旧的 `fidelity_first_model_design.md` 执行路线与 `data_and_experiments.md` 实验轴。三个明确变化：

- **基准优先**：先冻结「数据 + 评测协议」的 v1 基准，方法迭代只在 Python 评测框架内进行。MethodBundle/Slang/viewer 是独立的部署轨道，每个研究阶段收尾时对当期最优候选执行一次，不阻塞日常实验。`evaluate()` 返回线性 `f`、`prepare/evaluate` 划分、单 query 随机访问这些运行时合同保留为**设计期静态约束**，每个候选注册时检查，不需要每轮跑 shader。
- **解除研究期成本预算**：旧 `C_eval≤65k MAC`、`B_shared≤512KiB`、`B_asset≤512B` 是部署 gate，不再作为研究期淘汰条件。每个候选按 S/M/L 三档容量测「容量–质量曲线」，成本只记录。部署压缩（蒸馏、量化、字典化）是后置的独立阶段。
- **数据规模升级**：旧 E1/E2 数据（约 24 个 state、8 万条 query、几 MB）只能支撑「同一状态下未见方向」的初步判断，不足以支撑长尾分位数或任何未见状态结论。正式语料按第 2 节密度表生成，GB 级。
...
- **参数式族**（LayerStack、OpenPBR 常量参数资产）：从源参数直接推出。规则示例：`min(alpha) < 0.05 → S`；任一界面透射 → `T`；界面数 ≥ 2 → `M`。
- **测量/资产式族**（MERL、MaterialX）：源参数不可用或不可信，用低成本 E0 probe 实测：取 4 个代表性 `wo`，每个采 4,096 个均匀 `wi`，按 top-1% 积分能量占比与峰半宽估计分级。probe 成本远低于正式采集，分级结果随资产 manifest 版本化。

采样密度取所有命中标记的最大要求。

### 为什么必须 peak-aware，而不是加大均匀采样

半宽 σ（弧度）的峰在半球上占立体角约 `π σ²`，均匀采样命中概率约 `σ²/2`。要在峰内得到 ~20 个样本，纯均匀需要约 `40/σ²` 条：σ=2° 时约 1.6 万条尚可接受，σ=0.1° 时约 650 万条完全不可行。所以密度表的原则是：**均匀分量只负责能量背景与宽结构，窄峰覆盖全部来自以实际峰位为中心的多尺度 vMF proposal**。这套 proposal 机制（`ncls.e0-peak-grazing-mixture@2`、`ncls.materialx-local-normal-peak@1`、E2 的多界面峰 patch）已实现并通过 E0 审计，v1 基准直接沿用，只改密度参数。
...
| +T | 追加 32 个透射侧 `wo` | ×1.5 | 追加 25% 透射峰 + 10% 临界角带分量 |
| +M | 与 S 相同 | 与 S 相同 | vMF 中心改为实测峰位（按 E2 的高分辨率 probe 流程校准），每峰三尺度 |
...
以下事实来自已完成的 E0 校准，直接作为 v1 参数：

- 逐 query-group 自适应采样：目标 relative SE 全局 p95 ≤ 0.04、最坏 query group ≤ 0.10；cap 262,144 合并样本/query group（仅高方差 state 用到）。三组 LayerStack 状态的实测噪声曲线：合并 16,384 样本时 relative SE p95 ≈ 0.34，524,288 时 ≈ 0.12，1,048,576 时 ≈ 0.085——固定预算不可行，自适应必须保留。
- 单 dispatch batch ≤ 4,096（8,192 及以上在 Windows 触发过 TDR；安全值与方向数、层深相关，随生成配置记录）。
- 峰覆盖验收沿用 `ncls.e0-supervision-entry@6`：集中 query（top-1% 积分能量占比 ≥ 0.1）的 peak spacing p95 ≤ 2°、掠射比例 ≥ 0.08、OpenPBR 透射比例 ≥ 0.25。四族现行 proposal 均已通过该 gate，实测集中峰 spacing 0.06°–1.9°。
- 双 replica 落盘保留，replica 差异与 SE 一起进审计。

### 2.4 语料规模与生成策略

| 族 | 状态/资产规模 | 估算 |
|---|---|---|
| LayerStack | 24–32 个结构 family（层数 1–4 × 界面类型 × 有无介质 × 各向异性），每个 family 用 Sobol/LHS 采 8–12 个连续参数状态，共 **250–350 states** | 每 state 约 10–15 MB（含稠密切片），合计约 **3–5 GB** |
...
每条 query 落盘约 48–64 B（`wi`、mean、SE、PDF、权重、flags）。生成策略：
...
**关于旧数据**：现有几 MB 级 E1/E2 H5 保留为回归与 smoke 数据；任何写入实验注册表的正式比较必须使用 v1 语料。
...
- test 状态总数 ≥ 50。理由：报告 state 分布的 p95 至少需要几十个样本；旧 E2 用 2 个 test state 报告 p95 没有统计意义，这类结论一律作废。
...
旧 3k/8k step 实验在新语料规模下属于快速档。checkpoint 选择：validation 上的 solid-angle normalized L1 state-median（决胜看 p95）；test 只在配置冻结后读取一次。
...
split/hash/泄漏检查、输出 finite、范围符合族颜色合同、checkpoint 可恢复、fitted 统计只来自 train。这一层就是现行 E0/审计机制，保留。
...
log-domain error、峰位角（到 95% 峰高支持集的最近角距，沿用 E2 修正）、峰高比、top-energy recall、source-aware reciprocity deviation、按分级/族/state 的误差分布、稠密切片上的峰形对比图。

### 5.4 诊断量（只解释，不考核）

`model error / reference SE` 比值降级为诊断量。**旧的 ≤ 6 kill gate 废除**：该比值在 SE→0 的 query 上发散，且与感知质量无对应关系；它曾驱动 SE-floor loss、CVaR、refinement sweep 等一系列在阈值上无法收敛的实验，属于对可疑指标的过度优化。绝对误差与 SE 同时列出即可。
...
| **P5 及以后** | spatial latent/LOD、matched sampler、integration | MaterialX 扩容 | 原 E5/E6 内容不变 |
...
- **对照要求**：每个结论必须有 matched 对照（同数据、同预算、只差声明的机制组）。允许一次实验同时改变一组相关机制（例如「chart + FiLM」作为一个 bundle），但 bundle 内要保留能识别主要贡献的消融。**取消**「每轮只允许一个变化」的旧规则——它是模型长期停留在最小结构的直接原因。
