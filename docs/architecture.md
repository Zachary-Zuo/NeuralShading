...
- 当前方法阶段按 `docs/research/experiment_framework.md` 的基准优先路线推进：先冻结语料与评测协议，再在稳定框架内比较 `docs/research/model_candidates.md` 的候选；Slang parity/成本与 viewer 证据由每阶段收尾一次的部署轨道提供。sampler、环境积分和完整系统 benchmark 仍在 evaluator 与 compiler 主线稳定后展开。
