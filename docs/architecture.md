...
项目的根本目标仍然是：把限定原子词汇内的可编辑多层材质，转换为固定成本、可由实时光照系统使用的散射实现，并用随机游走参考解验证未见材质组合上的误差。

## 统一术语

- **随机游走参考解**（reference）：高采样 Monte Carlo 实现，用来生成监督、验证公式和提供 viewer 左侧参考图像。新代码和文档不再使用 `teacher` 作为名称。
...
1. **数据采集**：生成材质族和局部状态，通过随机游走参考解获得方向响应，写入可恢复、可验证的数据集。
...
数据集 → Python 学习与评测 → MethodBundle → Windows viewer
MaterialProgram → 随机游走参考解 ─────────────→ Windows viewer
```
...
`MaterialProgram` 是公共输入。它能表达有类型节点、资源和预留的表面/介质/发光/透明度/位移输出。第一阶段只实现可规范化为线性反射层栈的子集，但以后增加纹理、混合或新的原子节点时不改变顶层文件格式。

### 规范化层：内部 IR

数据采集和拟合后端不直接解释任意作者图。公共核心先验证节点和资源，再规范化为带版本的内部 IR。第一阶段的主要 IR 是 `LayerStackIR`，它不是公共材质格式。
...
- 随机游走参考解、数据采集和 viewer 左侧复用同一套 reference shader。
